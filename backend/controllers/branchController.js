...
const getBranches = async (req, res) => {
    try {
        const branches = await Branch.find({ gymId: req.user.gymId }).lean();

        // Attach member/revenue counts per branch
        const enriched = await Promise.all(branches.map(async (b) => {
            const memberCount = await Member.countDocuments({ gymId: req.user.gymId, branchId: b._id });
            const revenueAgg = await Payment.aggregate([
                { $match: { gymId: req.user.gymId, branchId: b._id } },
...
const createBranch = async (req, res) => {
    try {
        ...
        const branch = await Branch.create({ name, address, phone, email, managerName, gymId: req.user.gymId });
...
const deleteBranch = async (req, res) => {
    try {
        const branch = await Branch.findOneAndDelete({ _id: req.params.id, gymId: req.user.gymId });
        if (!branch) return res.status(404).json({ message: 'Branch not found' });
...
const getBranchMembers = async (req, res) => {
    try {
        const members = await Member.find({ gymId: req.user.gymId, branchId: req.params.id })
